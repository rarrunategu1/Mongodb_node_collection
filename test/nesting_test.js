...
describe('Nesting records', function(){
    ...
    beforeEach(function(done){ //this beforeEach makes it so we don't keep creating the same author
        ...
        //add book to books array
              ...
              record.save().then(function(){
                  Author.findOne({name: 'Patrick Rothfuss'}).then(function(result){
                      assert(result.books.length === 2); //2 because we're adding on the extra book
                      done();
                  });
              });
           });
